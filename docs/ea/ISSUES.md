...
79. **Phase 3 LSTM — behaviorální klonování s trajektoriovými advantagemi nefunguje: credit assignment problem** — po opravě fyzikálních mezí perturbací (issue #70), zavedení timestep masek (issue #71) a přechodu na min-max normalizaci advantagí (issue #78) model stále kolabuje na predikci ≈ 1.0 pro všechny timestepy; scatter plot ukazuje r=0.030 pro lr_factor a r=0.079 pro radius_factor (v podstatě nulová korelace); MAE ≈ 0.08 odpovídá predikci průměru výstupního rozsahu [0.75, 1.25]; residuální distribuce: ostrý spike na 0 (model predikuje 1.0, cíl je ≈ 1.0) + plochý ocas −0.25 až +0.25 bez struktury; kořenová příčina je strukturální — trajektoriová advantage je přiřazena celé trajektorii, ale model se musí naučit co dělat v každém jednotlivém timestepsu; pokud je trajektorie dobrá díky perturbaci v timestepsu 200, model zároveň dostane signál napodobovat nesouvisející perturbaci v timestepsu 100, protože obě sdílí stejnou váhu; průměrování přes ~152 přeturbovaných timestepů s náhodnou perturbací zrušuje konzistentní gradient; advantage quartile plot ukazuje nesouvisející vzory (lr MAE roste s advantage, radius klesá) — čistý šum; **tři možná řešení**: (A) per-timestep advantage — spustit každou perturbaci izolovaně (jedna perturbace na trajektorii), změřit delta MQE pro ten konkrétní zásah → přímý signál pro daný timestep bez credit assignment problému; (B) globální controller — místo per-checkpoint predikcí predikovat jeden pár (lr_scale, radius_scale) pro celý běh → jednodušší learning problem bez sekvence; (C) výrazně víc dat — 560 trajektorií s 152 přeturbovanými timestepy nestačí pro dekódování credit assignment přes ~300 epoch SOM tréninku; řešení A je strukturálně nejčistší a přímo adresuje root cause.

80. **Phase 3 vizualizace zahrnovala padding a nepřeturbované timestepy — výsledky fáze 2 a 3 vizuálně nerozeznatelné** — `plot_scatter_p3`, `plot_residuals_p3` a `plot_advantage_p3` zploštovaly všechny timestepy včetně paddingu (y=0.0) a nepřeturbovaných pozic (lr_f=1.0, rad_f=1.0, msk=0); z celkových 588 timestepů na trajektorii bylo 74 % buď padding nebo nepřeturbované; scatter byl dominován masou bodů s actual≈1.0 → predikce≈1.0 vypadala jako korelace; residuální distribuce měla falešný spike na +0.5 (padding artefakt, issue #72); výsledky vizuálně odpovídaly Phase 2 grafikám, ne skutečnému výkonu Phase 3 controlleru; opraveno: `load_test_data_p3` načítá `msk_test.npy`, přidána pomocná funkce `_flatten_perturbed(y_test, y_pred_list, msk_test, action_idx)` která konkatenuje pouze timestepy kde `msk_test[j] > 0`; všechny tři plot funkce rozšířeny o `msk_test` parametr a title obsahuje "(perturbed timesteps only)"; `run_single_p3` vypisuje počet přeturbovaných timestepů při spuštění.
...
85. **`dead_neuron_ratio` přesunuto z Pareto cílů do constraint — nahrazeno Spearmanovým ρ jako globální topologická metrika** — `dead_neuron_ratio` jako třetí Pareto cíl vytvářel selekční tlak směrem k malým mapám (menší mapa → menší dead ratio bez zlepšení topologie); tato informace je navíc již zachycena constraint violation systémem (viz body 43/44/48) — jako Pareto cíl tak byla redundantní a deformovala geometrii fronty; TE (topographic error) detekuje pouze lokální sousedské chyby — globálně zmačkaná mapa s několika rozpadlými sousedstvími může mít nízké TE i při špatné globální topologii (typicky multi-cluster data, kde každý cluster leží ve svém koutě mapy); nahrazení Spearmanovým ρ: korelace mezi pairwise vzdálenostmi vah v datovém prostoru a pairwise vzdálenostmi v grid prostoru (fyzické euklidovské vzdálenosti pro obě topologie); ρ blízko 1.0 = globální manifold je rozvinutý korektně, ρ blízko 0 nebo záporné = mapa zmačkaná; Spearmanův rank neví o absolutních škálách — metrika je dataset-size-independent na rozdíl od T&C; cíl v NSGA-II: `1 − ρ` (nižší = lepší, konzistentní s MQE ratio a TE); implementace: metoda `calculate_topological_correlation()` přidána do `som.py`; `ea.py` objectives přešly z `[mqe_ratio, TE, dead_ratio]` na `[mqe_ratio, TE, 1−ρ]`; `dead_neuron_ratio` zůstává jako constraint (Deb 2002 constrained dominance) s graduated threshold (viz bod 48); do `results.csv` a `pareto_front.csv` přidán sloupec `raw_topo_corr`.

86. **`calculate_topological_correlation` používala diskrétní cube-Manhattan vzdálenosti pro hex — ties snižovaly přesnost Spearmanova ranku** — grid vzdálenosti v cube souřadnicích jsou celá čísla (1, 2, 3...); na mapě 8×8 se 2016 páry neuronů tvoří shluky ties (např. vzdálenost 3 pokrývá stovky párů); Spearman seřadí ties arbitrárně → šum v ranku; dopad byl omezený (Spearman umí pracovat s ties), ale zbytečný; opraveno: hex mřížka nyní počítá fyzické euklidovské vzdálenosti `x_phys = j + 0.5*(i%2)`, `y_phys = i*√3/2` — páry se stejnou "topologickou vzdáleností" mají různé fyzické vzdálenosti (√3 ≈ 1.732 vs 2.0 pro skok 2), prakticky žádné ties; square topologie zůstává beze změny (euklidovská od začátku); obě topologie nyní konzistentně pracují s fyzickým prostorem; změna je izolovaná v `calculate_topological_correlation()` — cube_coords pro TE, tréninková sousedská funkce a `_grid_edges()` jsou nedotčeny.
