...
def _clean_dataframe_boundaries(df: pd.DataFrame) -> pd.DataFrame:
    initial_rows = df.shape[0]
    df_cleaned = df.dropna(how='all')
    if df_cleaned.shape[0] < initial_rows:
        log_message("SYSTEM", f"Removed {initial_rows - df_cleaned.shape[0]} empty rows from data.")
        pass
    return df_cleaned


def _read_csv_robust(input_path: str, delimiter: str = ',', **kwargs) -> pd.DataFrame:
    try:
        df = pd.read_csv(input_path, delimiter=delimiter, skipinitialspace=True, skip_blank_lines=True, **kwargs)

        df = _clean_dataframe_boundaries(df)
...
def validate_input_data(input_path: str, config_settings: dict) -> pd.DataFrame:

    if not os.path.exists(input_path):
        raise FileNotFoundError(f"Vstupní soubor '{input_path}' nebyl nalezen.")

    try:
        df = _read_csv_robust(input_path, delimiter=config_settings.get("delimiter", ","))
...
def preprocess_data(df: pd.DataFrame, config_settings: dict, working_dir: str) -> str:
    ...
    for col in data.columns:
        ...
        if col.startswith('id_') or col.endswith('_id') or col == config_settings.get('primary_id'):
            ...
            log_message("SYSTEM", f"Column '{col}': identified as categorical (ID column).")
            continue
        ...
        if pd.api.types.is_numeric_dtype(series):
            if n_uniques <= config_settings.get("categorical_threshold_numeric", 30):
                processed[col] = pd.factorize(series.fillna(""), sort=True)[0]
                categorical_info['categorical_column'].append(col)
                log_message("SYSTEM", f"Column '{col}': identified as categorical (numerical with {n_uniques} unique values).")
            else:
                num = pd.to_numeric(series, errors="coerce")
                processed[col] = num
                categorical_info['numerical_column'].append(col)
                log_message("SYSTEM", f"Column '{col}': identified as numerical ({n_uniques} unique values).")
        else:
            if n_uniques <= config_settings.get("categorical_threshold_text", 30):
                processed[col] = pd.factorize(series.fillna(""), sort=True)[0]
                categorical_info['categorical_column'].append(col)
                log_message("SYSTEM", f"Column '{col}': identified as categorical (text with {n_uniques} unique values).")
                ...
                log_message("SYSTEM", f"Column '{col}': identified as string/high-cardinality categorical ({n_uniques} unique values).")
    ...
    if not processed.empty:
        ...
        log_message("SYSTEM", f"Data normalized using MinMaxScaler.")
    else:
        normalized_df = pd.DataFrame()
        log_message("SYSTEM", "No numerical or categorical data to normalize.")

    if id_data is not None:
        # TODO - check if primary_id is not calculated in SOM analysis
        normalized_df = pd.concat([id_data, normalized_df], axis=1)
        log_message("SYSTEM", f"Primary ID '{primary_id_col}' re-attached to normalized data.")
    ...
    log_message("SYSTEM", f"Normalized data saved to '{output_path}'.")

    return output_path, normalized_df, categorical_info  # Vrátíme i DataFrame a info